  ...
  tagList(
    div(
      ...
      div(
        ...
        div(
          style = "display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 1.5em;",
          
          # Output-Focused Strategies
          div(
            style = "padding: 1em; background: white; border-radius: 4px; border: 1px solid #D2D0CE;",
            h4(
              style = "color: #006B75; margin-top: 0; margin-bottom: 0.75em; font-size: 1em;",
              icon("bullseye"),
              " Output-Focused"
            ),
            checkboxInput(
...
mod_tradeoff_explorer_server <- function(id, app_data) {
  moduleServer(id, function(input, output, session) {
    ...
    output$radar_chart <- renderGirafe({
      ...
      radar_data <- data %>%
        ...
        mutate(
          ...
          # Set factor order for clockwise flow: Top → Right → Bottom-Right → Bottom-Left → Left
          objective = factor(objective, levels = c(
            "Total Approvals",
            "Speed to Market", 
            "Capital Efficiency",
            "Predictability",
    ...
    # -------------------------------------------------------------------------
    # OUTPUT: Interpretation Text (DYNAMIC - updates with selection)
    # -------------------------------------------------------------------------
    output$interpretation_text <- renderUI({
      data <- selected_scenarios()
      req(nrow(data) >= 2)
      
      # Force reactivity by accessing the data
      n_scenarios <- nrow(data)
      
      # Calculate insights from SELECTED scenarios only
      ...
      most_learning <- data %>% slice_max(as.numeric(phase1_assets), n = 1)
      
      tagList(
        p(
          style = "color: #323130; line-height: 1.8;",
          strong("Key Insights from Your ", n_scenarios, " Selected Scenarios:")
        ),
        tags$ul(
          style = "color: #323130; line-height: 1.8;",
          tags$li(
          ...
          tags$li(
            strong("Most Learning Options: "),
            most_learning$scenario_name, " (",
            as.numeric(most_learning$phase1_assets), " Phase 1 assets)"
          )
        ),
        p(
